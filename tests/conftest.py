    ...
    with initialize_config_dir(config_dir=_CONFIG_DIR, version_base=None):
        cfg = compose(
            ...
            overrides=[
                ...
                "output.mode=email",
                "output.zotero.collection_path_parts=['泛读']",
                "output.zotero.write_note=true",
                "output.pdf.enabled=false",
                "output.pdf.dir=/tmp/zotero-pdf-tests",
                "scorer.enabled=false",
                "scorer.final_top_k=10",
                "scorer.model=gpt-4o-mini",
                "scorer.temperature=0.2",
                "scorer.prompt.system_prompt_path=config/scorer_system_prompt.txt",
                "scorer.prompt.user_prompt_path=config/scorer_user_prompt.txt",
                "scorer.prompt.schema_path=config/scorer_schema.json",
                "scorer.note.prompt_path=config/reading_note_prompt.txt",
                "scorer.metrics.write_txt=false",
                "scorer.metrics.output_dir=/tmp/zotero-pdf-tests",
            ],
        )
    return cfg
